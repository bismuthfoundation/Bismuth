class Fork():
    ...
    def check_postfork_reward_testnet(self, db_handler):
        #ram
        try:
            db_handler.execute_param(db_handler.c, "SELECT reward FROM transactions WHERE block_height = ?", (self.POW_FORK_TESTNET,))
            self.FORK_REWARD_TESTNET = db_handler.c.fetchone()[0]
        except:
            #hdd in case we have not saved yet
            db_handler.execute_param(db_handler.h, "SELECT reward FROM transactions WHERE block_height = ?", (self.POW_FORK_TESTNET,))
