        ...
        try:
            ...
            for block in block_array_data:
                ...
                for tx_index, transaction in enumerate(block):
                    ...
                    transaction_list_converted.append((tx.received_timestamp,
                                                       tx.received_address,
                ...
                for tx_index, transaction in enumerate(block):
                    ...
                    if tx_index == block_array.tx_count - 1:
                        ...
                        if previous_block.block_height <= 10000000:
                            ...
                            if node.last_block >= fork.POW_FORK or (node.is_testnet and node.last_block >= fork.POW_FORK_TESTNET):
                                ...
                                mining_reward = 15 - (quantize_eight(block_array.block_height_new) / quantize_eight(1000000 / 2)) - Decimal("0.8")
                    ...
                    block_transactions.append((str(block_array.block_height_new), str(db_timestamp), str(db_address),
                                               str(db_recipient), str(db_amount), str(db_signature),
                                               str(db_public_key_hashed), str(block_array.block_hash), str(fee),
                ...
                if node.is_testnet or block_array.block_height_new >= 843000:
                    ...
                    if int(block_array.block_height_new) % 10000 == 0:  # every x blocks
                        ...
                        staking.staking_payout(db_handler.conn, db_handler.c, db_handler.index, db_handler.index_cursor,
                                               block_array.block_height_new, float(miner_tx.q_block_timestamp),
                                               node.logger.app_log)
                        staking.staking_revalidate(db_handler.conn, db_handler.c, db_handler.index,
                                                   db_handler.index_cursor, block_array.block_height_new,
                                                   node.logger.app_log)
                ...
                node.logger.app_log.warning(f"Valid block: {block_array.block_height_new}: "
            ...
            node.plugin_manager.execute_action_hook('digestblock',
                                                    {'failed': block_array.failed_cause, 'ip': peer_ip,
                                                     'deltat': delta_t, "blocks": block_array.block_count,
