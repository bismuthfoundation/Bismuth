...
def aliases_update(node, db_handler_instance):
    ...
    db_handler_instance.index_cursor._execute("SELECT block_height FROM aliases ORDER BY block_height DESC LIMIT 1;")
    try:
    ...
    db_handler_instance.h._execute("SELECT block_height, address, openfield FROM transactions WHERE operation = ? AND block_height >= ? AND reward = 0 ORDER BY block_height ASC, timestamp ASC;", ("alias:register", alias_last_block,))
    ...
    for openfield in result:
        ...
        try:
            db_handler_instance.index_cursor._execute("SELECT * from aliases WHERE alias = ?", (openfield[2],))
            ...
            db_handler_instance.index_cursor._execute("INSERT INTO aliases VALUES (?,?,?)", (openfield[0], openfield[1], openfield[2]))
