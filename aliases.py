...
def aliases_update(node, db_handler_instance): 
    ...
    db_handler_instance.h.execute("SELECT block_height, address, openfield FROM transactions WHERE operation = ? AND block_height >= ? ORDER BY block_height ASC, timestamp ASC;", ("alias_register", alias_last_block,))
