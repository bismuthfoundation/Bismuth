...
def aliases_update(node, db_handler_instance):
    ...
    db_handler_instance.h._execute("SELECT block_height, address, openfield FROM transactions WHERE openfield LIKE ? AND block_height >= ? ORDER BY block_height ASC, timestamp ASC;", ("alias=" + '%',) + (alias_last_block,))
    ...
    for openfield in result:
        ...
        try:
            db_handler_instance.index_cursor._execute("SELECT * from aliases WHERE alias = ?", (alias,))
            ...
            db_handler_instance.index_cursor._execute("INSERT INTO aliases VALUES (?,?,?)", (openfield[0], openfield[1], alias))
